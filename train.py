...
def compute_class_weights(df):
    ...
    return torch.tensor(age_class_weights, dtype=torch.float32), torch.tensor(race_class_weights, dtype=torch.float32)
...
def train_model(train_module, data_module):
    ...
    wandb_logger = WandbLogger(project="UTK_Age_Prediction", save_dir='./lightning_logs',
                               name="resnet101_onecycle_class_weighted")
...
if __name__ == '__main__':
    ...
    age_weights, race_weights = compute_class_weights(full_df)
    print(age_weights, race_weights)
    train_module = AgePrediction(age_weights, race_weights)
